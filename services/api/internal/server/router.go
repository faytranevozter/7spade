...
func NewRouter(cfg *config.Config, db *sql.DB, rdb *cache.RedisClient) *gin.Engine {
	...
	r.POST("/auth/mobile/google", authRL, oauthHandler.MobileGoogle)
	r.POST("/auth/mobile/telegram", authRL, oauthHandler.MobileTelegram)
	r.POST("/auth/mobile/telegram/start", authRL, oauthHandler.MobileTelegramStart)
	r.GET("/auth/mobile/telegram/callback", authRL, oauthHandler.MobileTelegramCallback)
	r.POST("/auth/mobile/telegram/exchange", authRL, oauthHandler.MobileTelegramExchange)
