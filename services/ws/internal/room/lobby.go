...
func (room *room) finalizeLobbyLeave(target *player, token int) {
	...
	notify := room.removeAndNotifyLobbyLeaveLocked(target, false)
	room.mu.Unlock()
	notify()
}
...
func (room *room) handleKick(initiator *player, targetSlot int) {
	...
	// roster to the remaining players after unlocking. kick=true records
	// the kick server-side so the player can't rejoin via the HTTP join path.
	notify := room.removeAndNotifyLobbyLeaveLocked(target, true)
	room.mu.Unlock()
	notify()
}

// removeAndNotifyLobbyLeaveLocked removes a seat and returns off-lock notifications.
// Caller must hold room.mu; the helper leaves it held.
func (room *room) removeAndNotifyLobbyLeaveLocked(target *player, kick bool) func() {
