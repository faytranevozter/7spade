...
// Native port of web/src/components/StatCards.tsx. A read-only set of labelled
// stat groups, each a wrap-grid of tiles. Derived rates guard a zero divisor.
...
function StatGroup({ group }: { group: Group }) {
  return (
    <View>
      <Text className="mb-2 font-mono text-[11px] uppercase tracking-wider text-spade-gold">{group.title}</Text>
      <View className="flex-row flex-wrap gap-3">
        {group.tiles.map((tile) => (
          ...
          >
            <Text className="font-mono text-[10px] uppercase text-spade-gray-3">{tile.label}</Text>
            <Text className="mt-1 text-2xl font-medium text-spade-cream">{tile.value}</Text>
          </View>
        ))}
      </View>
    </View>
