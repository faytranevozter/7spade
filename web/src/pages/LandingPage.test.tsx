...
test('landing page explains app purpose and links to legal pages', () => {
  ...
  expect(screen.getByRole('heading', { name: /A table for every group/i })).toBeInTheDocument()
  expect(screen.getByText(/custom rooms for 2–8 players/i)).toBeInTheDocument()
  expect(screen.getByText(/complete achievements and events/i)).toBeInTheDocument()
  expect(screen.getByText(/move-by-move replays/i)).toBeInTheDocument()
