...
// The `id` values must stay in sync with the server allowlist in
// services/ws/server.go (allowedEmotes); the server rejects any id not on its
// list. `glyph` is what renders in the picker and the bubble; `label` is the
// accessible name (and the visible text for word emotes like "GG").
export type Emote = {
  id: string
  label: string
  glyph: string
