...
export function SettingsPage() {
  ...
  const [dailyLoginXP, setDailyLoginXP] = useState<DailyLoginXP>({
    xp_base: 10,
    xp_step: 5,
    xp_max: 50,
  })
  const [persistedDailyLoginXP, setPersistedDailyLoginXP] =
    useState<DailyLoginXP>({ xp_base: 10, xp_step: 5, xp_max: 50 })
        ...
        if (!cancelled) {
          const values = Object.fromEntries(
            loaded
              ...
              .map((setting) => [setting.key, setting.value as boolean]),
          )
          setSettings(values)
          setPersisted(values)
          const integers = Object.fromEntries(
            loaded
              ...
              .map((setting) => [setting.key, setting.value as number]),
          )
          if (
            dailyLoginXPKeys.xp_base in integers &&
            dailyLoginXPKeys.xp_step in integers &&
            dailyLoginXPKeys.xp_max in integers
          ) {
            const xp = {
              xp_base: integers[dailyLoginXPKeys.xp_base],
              xp_step: integers[dailyLoginXPKeys.xp_step],
              xp_max: integers[dailyLoginXPKeys.xp_max],
            }
            setDailyLoginXP(xp)
  ...
  async function save(event: FormEvent, key: string, title: string) {
    ...
    if (
      ...
      (settings[key] === persisted[key] &&
        (key !== 'daily_login' ||
          JSON.stringify(dailyLoginXP) ===
            JSON.stringify(persistedDailyLoginXP))) ||
      !reason
    )
      return
    ...
    try {
      if (key === 'daily_login') {
        ...
        if (dailyLoginXP.xp_max > persistedDailyLoginXP.xp_max)
          updates.push([dailyLoginXPKeys.xp_max, dailyLoginXP.xp_max])
        if (dailyLoginXP.xp_base !== persistedDailyLoginXP.xp_base)
          updates.push([dailyLoginXPKeys.xp_base, dailyLoginXP.xp_base])
        if (dailyLoginXP.xp_step !== persistedDailyLoginXP.xp_step)
          updates.push([dailyLoginXPKeys.xp_step, dailyLoginXP.xp_step])
        if (dailyLoginXP.xp_max < persistedDailyLoginXP.xp_max)
          updates.push([dailyLoginXPKeys.xp_max, dailyLoginXP.xp_max])
      ...
      setStatuses((current) => ({
        ...
        [key]:
          key === 'daily_login'
            ...
            : `${title} ${settings[key] ? 'enabled' : 'disabled'}`,
      }))
    } catch (cause) {
      setErrors((current) => ({
        ...current,
                      ...
                      {control.key === 'daily_login' ? (
                        <div className="border-admin-border-divider mt-6 grid gap-4 border-t pt-5 sm:grid-cols-3">
                          {(
                            ...
                            <label
                              key={field}
                              className="text-admin-ink grid gap-2 text-sm font-medium"
                            >
                              {label}
                              <input
                                type="number"
