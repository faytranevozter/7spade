...
import { getUser, reinstateUser, suspendUser, type UserDetail } from '../api/users'
...
export function UserDetailPage() {
  ...
  async function loadUser() {
    ...
    try {
      setDetail(await getUser(token, id))
      setMessage('')
    } catch (error) {
  ...
  return <section aria-labelledby="user-detail-heading">
    ...
    {detail ? <UserDetailPanel detail={detail} canModerate={admin?.permissions.includes('users.moderate') ?? false} reason={reason} expiresAt={expiresAt} onReasonChange={setReason} onExpiresAtChange={setExpiresAt} onUpdateSuspension={updateSuspension} /> : null}
  </section>
}

function UserDetailPanel({ detail, canModerate, reason, expiresAt, onReasonChange, onExpiresAtChange, onUpdateSuspension }: { detail: UserDetail; canModerate: boolean; reason: string; expiresAt: string; onReasonChange: (value: string) => void; onExpiresAtChange: (value: string) => void; onUpdateSuspension: () => Promise<void> }) {
  const { user } = detail
