...
test('invitee validates an invitation and creates an administrator account', async () => {
  ...
  const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
    ...
    if (url.endsWith('/auth/invitations/inspect') && init?.method === 'POST') {
    ...
    if (url.endsWith('/auth/invitations/accept') && init?.method === 'POST') {
      return new Response(JSON.stringify({ id: 'admin-2', email: 'new@example.com' }), { status: 200 })
    }
  ...
  const request = fetchMock.mock.calls.find(([url]) => String(url).endsWith('/auth/invitations/accept'))
  expect(request?.[1]?.body).toBe(JSON.stringify({ token: 'invite-token', display_name: 'New Operator', password: 'secure-password' }))
})

test('administrator enrolls MFA and receives recovery codes', async () => {
  window.location.hash = '#/security'
  const admin = { id: 'admin-1', email: 'ops@example.com', display_name: 'Operator', status: 'active', permissions: [], mfa_enrolled: false }
  vi.spyOn(globalThis, 'fetch').mockImplementation(async (input) => {
    const url = String(input)
    if (url.endsWith('/auth/refresh')) return new Response(JSON.stringify({ access_token: 'access', admin }), { status: 200 })
    if (url.endsWith('/auth/mfa/enroll')) return new Response(JSON.stringify({ secret: 'TOTPSECRET', uri: 'otpauth://totp/SevenSpade' }), { status: 200 })
    if (url.endsWith('/auth/mfa/confirm')) return new Response(JSON.stringify({ recovery_codes: ['code-one', 'code-two'] }), { status: 200 })
    throw new Error(`Unexpected request: ${url}`)
  })
