  ...
  const fetchMock = vi
    ...
    .mockImplementation(async (input) => {
      ...
      if (url.endsWith('/auth/refresh'))
        return new Response(JSON.stringify({ access_token: 'token', admin }), {
          status: 200,
        })
...
test('administrator signs in and sees the protected dashboard', async () => {
  const dashboard = {
    ...
    environment: 'staging',
    windows: {
      day: { from: '2026-08-28T00:00:00Z', to: '2026-08-29T00:00:00Z' },
      month: { from: '2026-08-01T00:00:00Z', to: '2026-09-01T00:00:00Z' },
    },
    ...
    daily: {
      ...
      games_completed: 1,
      games_abandoned: 0,
      average_game_duration_seconds: 125,
    },
    monthly: {
      registrations: 20,
      players: 33,
      ...
      games_completed: 12,
      games_abandoned: 1,
      average_game_duration_seconds: 245,
    },
  ...
  const fetchMock = vi
    ...
    .mockImplementation(async (input) => {
      ...
      if (url.includes('/users?'))
        return new Response(
          JSON.stringify({ users: [user], limit: 50, offset: 0 }),
          { status: 200 },
        )
      if (url.endsWith(`/users/${user.id}`))
        return new Response(
          JSON.stringify({
            user,
            providers: ['google'],
...
test('operator searches a game and adds a reasoned investigation note', async () => {
  ...
  const admin = {
    ...
    display_name: 'Operator',
    status: 'active',
    permissions: ['games.read', 'games.annotate'],
  }
  const game = {
    game_id: '10000000-0000-0000-0000-000000000001',
    room_id: 'room-1',
    room_name: 'Practice table',
    mode: 'classic',
  ...
  const fetchMock = vi
    ...
    .mockImplementation(async (input, init) => {
      ...
      if (url.includes('/games?'))
        return new Response(
          JSON.stringify({ games: [game], limit: 50, offset: 0 }),
          { status: 200 },
        )
      if (url.endsWith(`/games/${game.game_id}`))
        return new Response(
          JSON.stringify({
            game,
            players: [
      ...
      if (
        url.endsWith(`/games/${game.game_id}/notes`) &&
        init?.method === 'POST'
      )
        return new Response(
          JSON.stringify({
  ...
  await waitFor(() =>
    expect(
      fetchMock.mock.calls.some(
        ([input, init]) =>
          String(input).endsWith('/notes') && init?.method === 'POST',
      ),
    ).toBe(true),
  )
})
  ...
  const fetchMock = vi
    ...
    .mockImplementation(async (input) => {
      ...
      if (url.includes('/rooms?'))
        return new Response(
          JSON.stringify({ rooms: [room], limit: 50, offset: 0 }),
          { status: 200 },
        )
      if (url.endsWith(`/rooms/${room.id}`))
        return new Response(
          JSON.stringify({
            ...
            live: { available: false, reason: 'unavailable' },
          }),
          { status: 200 },
        )
      throw new Error(`Unexpected request: ${url}`)
        ...
        JSON.stringify({
          ...
          live: {
            ...
            summary: {
              ...
              phase: 'playing',
              players: [
                { user_id: 'player-1', display_name: 'Ace', connected: true },
                {
                  user_id: 'bot-1',
                  display_name: 'Robo',
                  connected: true,
...
test('operator opens user detail and advances user pagination', async () => {
  ...
  const fetchMock = vi
    ...
    .mockImplementation(async (input) => {
      ...
      if (url.endsWith('/sessions'))
        return new Response(JSON.stringify([]), { status: 200 })
      if (url.includes('/users?limit=50&offset=0'))
        return new Response(
          JSON.stringify({ users: firstPage, limit: 50, offset: 0 }),
          { status: 200 },
        )
      if (url.includes('/users?limit=50&offset=50'))
        return new Response(
          JSON.stringify({ users: [nextUser], limit: 50, offset: 50 }),
          { status: 200 },
        )
      if (url.endsWith(`/users/${firstPage[0].id}`))
        return new Response(
          JSON.stringify({
            user: firstPage[0],
            providers: null,
  ...
  render(<App />)
  fireEvent.click(
    await screen.findByRole('link', { name: /Player 1 @player1/ }),
  )
  expect(
  ...
  expect(
    fetchMock.mock.calls.some(([input]) =>
      String(input).includes('/users?limit=50&offset=50'),
    ),
  ).toBe(true)
})
...
test('moderator recovers a display name conflict without losing the attempted replacement', async () => {
  ...
  const admin = {
    ...
    status: 'active',
    permissions: ['users.read', 'users.moderate'],
  }
  const original = {
    id: '00000000-0000-0000-0000-000000000001',
    username: 'ace',
  ...
  const fetchMock = vi
    ...
    .mockImplementation(async (input, init) => {
      ...
      if (url.endsWith(`/users/${original.id}`) && !init?.method) {
        ...
        return new Response(
          JSON.stringify({
            user: detailCalls === 1 ? original : current,
            providers: [],
            stats: {},
            ratings: [],
            achievements: [],
      ...
      if (url.endsWith(`/users/${original.id}/display-name`))
        return new Response(
          JSON.stringify({ error: 'User changed since it was loaded' }),
          { status: 409 },
        )
      throw new Error(`Unexpected request: ${url}`)
    })
  ...
  render(<App />)
  fireEvent.change(await screen.findByLabelText('Replacement display name'), {
    target: { value: 'Clean Name' },
  })
  fireEvent.change(screen.getByLabelText('Moderation reason'), {
    target: { value: 'Inappropriate name' },
  })
  fireEvent.click(screen.getByRole('button', { name: 'Replace display name' }))

  expect(await screen.findByRole('alert')).toHaveTextContent(
    'Current name: Newer Name',
  )
  expect(screen.getByLabelText('Replacement display name')).toHaveValue(
    'Clean Name',
  )
  const updateCall = fetchMock.mock.calls.find(([input]) =>
    String(input).endsWith('/display-name'),
  )
...
test('administrator completes an MFA challenge before entering the shell', async () => {
  ...
  fetchMock.mockResolvedValueOnce(
    new Response(
      JSON.stringify({ mfa_required: true, challenge_token: 'challenge' }),
      { status: 202 },
    ),
  )
  fetchMock.mockResolvedValueOnce(
    new Response(JSON.stringify({ access_token: 'token', admin }), {
      status: 200,
    }),
  )
  render(<App />)
  ...
  const fetchMock = vi
    ...
    .mockImplementation(async (input, init) => {
      ...
      if (url.endsWith('/auth/refresh'))
        return new Response(
          JSON.stringify({
            access_token: dashboardCalls ? 'fresh' : 'expired',
            admin,
          }),
          { status: 200 },
        )
...
test('super administrator manages other administrators', async () => {
  ...
  vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
    ...
    if (url.endsWith('/auth/refresh'))
      return new Response(
        JSON.stringify({ access_token: 'token', admin: superAdmin }),
        { status: 200 },
      )
    if (url.endsWith('/dashboard'))
      return new Response(
    ...
    if (url.endsWith('/sessions'))
      return new Response(JSON.stringify([]), { status: 200 })
    if (url.endsWith('/admins') && !init?.method)
      return new Response(JSON.stringify([superAdmin, otherAdmin]), {
        status: 200,
      })
    if (url.endsWith('/roles') && !init?.method)
      return new Response(JSON.stringify(roles), { status: 200 })
    if (url.endsWith('/permissions'))
      return new Response(JSON.stringify(permissions), { status: 200 })
    if (url.endsWith('/admins/invite'))
      return new Response(
        JSON.stringify({
    ...
    if (url.includes('/admins/') || url.includes('/roles/'))
  ...
  expect(screen.getByRole('status')).toHaveTextContent(
    'Invitation created for new@example.com',
  )

  // 2. Change role
  fireEvent.change(screen.getByLabelText('Role for Moderator Admin'), {
    target: { value: 'role-op' },
  })
  await waitFor(() =>
    expect(screen.getByRole('status')).toHaveTextContent(
  ...
  await waitFor(() =>
    expect(screen.getByRole('status')).toHaveTextContent(
      'Administrator mod@example.com is now disabled',
    ),
  )
  ...
  fireEvent.click(
    screen.getByRole('button', { name: 'Save moderator permissions' }),
  )
  await waitFor(() =>
    expect(screen.getByRole('status')).toHaveTextContent(
...
test('operator filters the skin catalog and opens a skin detail', async () => {
  ...
  const skins = [
    ...
    },
    {
      ...
      name: 'Forest table',
      description: 'Deep green felt',
      asset_key: 'forest.png',
      display_order: 2,
      enabled: true,
  ...
  render(<App />)
  expect(
    await screen.findByRole('heading', { name: 'Skins' }),
  ).toBeInTheDocument()
  fireEvent.change(screen.getByLabelText('Search catalog'), {
    target: { value: 'forest' },
  })
  expect(
    screen.queryByRole('link', { name: 'Open Gold frame' }),
  ).not.toBeInTheDocument()
  fireEvent.change(screen.getByLabelText('State / visibility'), {
    target: { value: 'hidden' },
  })
  fireEvent.click(screen.getByRole('link', { name: 'Open Forest table' }))
  expect(
    await screen.findByRole('heading', { name: 'Forest table' }),
  ).toBeInTheDocument()
  expect(window.location.hash).toBe('#/skins/skin-2')
...
test('skin manager creates a draft and opens its asset pipeline', async () => {
  ...
  const admin = {
    ...
    display_name: 'Operator',
    status: 'active',
    permissions: ['skins.read', 'skins.manage'],
  }
  const created = {
    id: 'new-skin',
...
test('skin manager handles null collections from the API', async () => {
  ...
  expect(screen.getByText('No unlock rules configured.')).toBeInTheDocument()
...
test('skin catalog and detail show real assets, starter information, structured rules, and a hidden file input', async () => {
  ...
  const skin = {
    ...
    asset_key: 'starter.png',
    asset_url: 'https://cdn.example/starter.png',
    is_starter: true,
    display_order: 1,
    enabled: true,
    catalog_visible: true,
  ...
  render(<App />)
  expect(
    await screen.findByRole('img', { name: 'Starter frame skin' }),
  ).toHaveAttribute('src', skin.asset_url)
...
test('skin manager previews an uploaded asset before publishing it', async () => {
  window.location.hash = '#/skins/skin-1'
  const admin = {
    id: '1',
    email: 'ops@example.com',
  ...
  const fetchMock = vi
    ...
    .mockImplementation(async (input, init) => {
      ...
      if (url.endsWith('/skins/skin-1/revisions')) {
  ...
  fireEvent.change(await screen.findByLabelText('Asset file'), {
    target: { files: [file] },
  })
  expect(
    await screen.findByAltText('Unpublished skin preview'),
